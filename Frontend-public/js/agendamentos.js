...
const areaBloqueio = document.getElementById("areaBloqueio");

const btnAbrirBloqueio = document.getElementById("btnAbrirBloqueio");

const modalBloqueioOverlay =
  document.getElementById("modalBloqueioOverlay");

const btnFecharBloqueio =
  document.getElementById("btnFecharBloqueio");

const btnCancelarBloqueio =
  document.getElementById("btnCancelarBloqueio");
...
const mensagemBloqueio =
  document.getElementById("mensagemBloqueio");

const btnSalvarBloqueio =
  document.getElementById("btnSalvarBloqueio");

let clientes = [];
let assinaturasPlano = [];
let barbeirosDisponiveis = [];
let cargoUsuarioLogado = "";
...
function mostrarMensagemBloqueio(
  ...
  mensagemBloqueio.textContent = texto;
  mensagemBloqueio.dataset.tipo = tipo;
}
...
function configurarLimiteDataBloqueio() {
  if (!bloqueioData) {
    return;
  }
  ...
  const dataMaxima = `${anoAtual + 5}-12-31`;

  bloqueioData.min = dataMinima;

  bloqueioData.max = dataMaxima;

  const dataAtual = formatarDataInput(hoje);

  if (
    !bloqueioData.value ||
    bloqueioData.value < dataMinima ||
    bloqueioData.value > dataMaxima
  ) {
    bloqueioData.value = dataAtual;
  }
}

function atualizarDiasDaSemana() {
  const semana =
    semanaSelect.value === "proxima" ? 1 : 0;
...
async function carregarUsuarioLogadoParaBloqueio() {
  try {
    ...
    const podeGerenciarBloqueio =
      cargoUsuarioLogado === "ADMIN" ||
      cargoUsuarioLogado === "GERENTE";

    if (areaBloqueio) {
      areaBloqueio.style.display =
...
function preencherBarbeirosBloqueio() {
  if (!bloqueioBarbeiro) {
    return;
  }

  bloqueioBarbeiro.innerHTML =
...
function abrirModalBloqueio() {
  ...
  mensagemBloqueio.dataset.tipo =
    "";

  configurarLimiteDataBloqueio();

  preencherBarbeirosBloqueio();

  if (
    diaSelect.value &&
    diaSelect.value >= bloqueioData.min &&
    diaSelect.value <= bloqueioData.max
  ) {
    bloqueioData.value =
      diaSelect.value;
  }

  modalBloqueioOverlay.classList.add(
    "aberto",
  );

  modalBloqueioOverlay.setAttribute(
    "aria-hidden",
    "false",
  );
...
function fecharModalBloqueio() {
  modalBloqueioOverlay.classList.remove(
    "aberto",
  );
...
function validarFormularioBloqueio() {
  const data =
    bloqueioData.value;
  ...
  if (
    ...
    return `A data do bloqueio deve estar entre ${bloqueioData.min} e ${bloqueioData.max}.`;
  }
  ...
  if (
    ...
    return "Informe o horário inicial e o horário final.";
  }
...
async function criarBloqueio() {
  ...
  btnSalvarBloqueio.disabled = true;
  ...
  try {
    ...
    btnSalvarBloqueio.disabled = false;
  }
}
...
async function carregarClientes() {
  try {
    const resultado =
      await api.get(
        "/clientes?ativo=true&limite=100",
      );
...
async function carregarPlanosCliente(
  clienteId,
) {
  assinaturasPlano = [];
  ...
  try {
    ...
    if (!resultado?.success) {
      throw new Error(
        resultado?.message ||
          "Erro ao carregar planos do cliente.",
      );
    }
    ...
    assinaturasPlano.forEach(
      (assinatura) => {
        ...
        option.textContent =
          `${assinatura.plano.nome} - ${restantes} corte(s) restante(s)`;

        option.dataset.servicoId =
          assinatura.plano.servico_id;

        option.dataset.restantes =
          restantes;

        option.dataset.dataFim =
          assinatura.data_fim;

        planoSelect.appendChild(
          option,
        );
      },
    );
    ...
    mostrarMensagem(
      "Erro ao carregar os planos do cliente.",
      "danger",
    );
  }
}
...
function atualizarInformacaoPlano() {
  ...
  planoInfo.textContent =
    `${restantes} corte(s) restante(s) • Válido até ${dataFormatada}`;
}
...
function mostrarClientes(lista) {
  ...
  if (!lista.length) {
    ...
    clientesResultados.style.display =
      "block";
  ...
  lista.forEach(
    (clienteAtual) => {
      ...
      clientesResultados.appendChild(
        resultado,
      );
    },
  );
...
barbeiroSelect.addEventListener(
  "change",
...
diaSelect.addEventListener(
  ...
  () => {
    ...
    if (
      bloqueioData &&
      modalBloqueioOverlay &&
      modalBloqueioOverlay.classList.contains(
        "aberto",
      )
    ) {
      bloqueioData.value =
...
tipoPlano.addEventListener(
  "change",
  atualizarEstadoPlano,
);

planoSelect.addEventListener(
  "change",
  atualizarInformacaoPlano,
);
...
servicoSelect.addEventListener(
  ...
  () => {
    ...
    const assinatura =
      obterAssinaturaPlanoSelecionada();
...
form.addEventListener(
  ...
  async (evento) => {
    ...
    if (
      ...
      mostrarMensagem(
        "Selecione uma assinatura de plano válida.",
        "danger",
      );
    ...
    if (
      ...
      mostrarMensagem(
        "O serviço selecionado não faz parte do plano.",
        "danger",
      );
...
if (btnAbrirBloqueio) {
  btnAbrirBloqueio.addEventListener(
    "click",
    abrirModalBloqueio,
  );
}

if (btnFecharBloqueio) {
  btnFecharBloqueio.addEventListener(
    "click",
    fecharModalBloqueio,
  );
}
...
if (btnCancelarBloqueio) {
  btnCancelarBloqueio.addEventListener(
    "click",
    fecharModalBloqueio,
  );
}

if (modalBloqueioOverlay) {
  modalBloqueioOverlay.addEventListener(
    "click",
    (evento) => {
      if (
        evento.target ===
...
async function carregarDados() {
  ...
  await carregarUsuarioLogadoParaBloqueio();
